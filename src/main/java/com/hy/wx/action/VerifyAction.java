...
public class VerifyAction extends BaseAction {
    ...
    protected String message(final HttpServletRequest request, HttpServletResponse response, String signature, String timestamp, String nonce, @RequestParam(required = true) String openid, String encrypt_type, String msg_signature) throws ServletException, IOException {
    ...
    public Map call(HttpServletRequest request, HttpServletResponse response,String code, final String state) {
        ...
        try {
            if(resultMap.containsKey("data")) {
                response.sendRedirect(params[1].replace('~','#'));
            }else {
                response.sendRedirect("/wx/dist/index.html#/login");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
...
/*    @RequestMapping(value = "authToken")
    ...
    public Object getParams(HttpServletResponse response,String nostr) {
        Assert.notNull(nostr,"参数不能为null");
