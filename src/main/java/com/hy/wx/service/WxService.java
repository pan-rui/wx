...
import com.hy.wx.aes.WxBizMsgCrypt;
import com.hy.wx.http.CacheUtil;
import com.hy.wx.aes.AesException;
...
*/
    ...
    public Map respToken(String code, String state) {
        ...
        cacheUtil.setCacheOnExpire(openid+"_up", (String)tokenMap.get("refresh_token"), 3600*24*30-60);     //refresh_token 30天
        //后端查询
...
//            cacheUtil.hSetCache("USERINFO",openid,JSON.toJSONString(resultMap.get("data")));
            ...
            //TODO:设置用户Head缓存
            cacheUtil.hSetCache(CacheKey.WX_HEAD, CacheKey.USER_ID+openid,String.valueOf(userMap.get("id")));
            cacheUtil.hSetCache(CacheKey.WX_HEAD, CacheKey.OPENID+openid,openid);
            cacheUtil.hSetCache(CacheKey.WX_HEAD, CacheKey.USER_PHONE+openid,String.valueOf(userMap.get("phone")));
            return resultMap;
        }
    }
...
//            String token=reqToken(openid);
            ...
            String msgType= (String) reader.read(decryptMsg).selectObject("/xml/MsgType/text()");
            String content=(String)reader.read(decryptMsg).selectObject("/xml/Content/text()");
    ...
    public Map<String, Object> getHead(String nostr) {
        ...
        if (openId != null) {
            headInfo.put(CacheKey.OPENID, openId);
            headInfo.put(CacheKey.USER_ID, cacheUtil.hGetCache(CacheKey.WX_HEAD, CacheKey.USER_ID+openId));
            headInfo.put(CacheKey.USER_PHONE, cacheUtil.hGetCache(CacheKey.WX_HEAD, CacheKey.USER_PHONE+openId));
            headInfo.put(CacheKey.APP_VER, "USER-WeChat");
        }
        return headInfo;
    }
