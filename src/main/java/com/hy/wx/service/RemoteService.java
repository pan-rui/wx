...
public class RemoteService {
    ...
    public Map getUserInfo(String ddBB, String openId, String tenantId) {
        return JSON.parseObject(HttpUtil.execute(RemoteProtocol.USERINFO, ParamsMap.newMap("openId", openId)), Map.class);
    }

    public Map validAndUser(String phone, String projectCode, String openId,Map<String,Object> wxUser) {
        return JSON.parseObject(HttpUtil.execute(RemoteProtocol.ValidAndUser, ParamsMap.newMap("params", ParamsMap.newMap("openId", openId).addParams("projectCode", projectCode).addParams("phone", phone).addParams("wxUser", wxUser))));
    }
    ...
    public Map addUser(String openId,String tenantId,String projectCode,String ddBB,Map userInfo) {
        ...
        try {
            resultMap = JSON.parseObject(new String(HttpUtil.execute(RemoteProtocol.ADDUSER, ParamsMap.newMap("params", ParamsMap.newMap("openId", openId).addParams("projectCode", projectCode).addParams("ddBB", ddBB).addParams("tenantId", tenantId).addParams("userInfo", userInfo))).getBytes("ISO-8859-1"), "UTF-8"));
    ...
    public Map downImg(String openId, String tenantId, String ddBB, String projectCode,String serverId,int isFront,String accToken) {
        return JSON.parseObject(HttpUtil.execute(RemoteProtocol.DOWNIMG, ParamsMap.newMap("openId", openId).addParams("tenantId", tenantId).addParams("ddBB", ddBB).addParams("projectCode", projectCode).addParams("serverId",serverId).addParams("accToken",accToken).addParams("isFront",isFront)),Map.class);
    }

    public Map aliNotify(Map params) {
        return JSON.parseObject(HttpUtil.execute(RemoteProtocol.ALINOTIFY, ParamsMap.newMap("params", params)),Map.class);
    }
}
